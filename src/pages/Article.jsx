...
import { Icon, Item, Modal, Image } from 'semantic-ui-react';
...
// modules
import { getSingleArticle, likeArticle } from '../store/modules/article';
...
export class Article extends Component {
  ...
  likeArticleHandler = () => {
    const { id, slug } = this.props.article;
    return this.props.likeArticle(this.checkLikeState(), { id, slug });
  };

  checkLikeState = () => {
    const { id } = decodeToken(getToken());
    const { likes } = this.props.article;
  ...
  renderIcon = () => {
    if (this.checkLikeState() === 'unlike') return LikeIcon;
    return UnLikeIcon;
  };
        ...
        <div className="article-container" data-test="article">
          ...
          <div className="sidebar actions justify-content-center">
            <div className="fixed-pos">
              ...
              >
                {article.likes && <Image src={this.renderIcon()} />}
              </Button>
            </div>
          </div>
        </div>
...
const mapStateToProps = (state, props) => {
  ...
  return {
    ...
    likes: state.article.articles[articleId]
      ? state.article.articles[articleId].likes || []
      : [],
  };
};
